...
def checkMultiBroadcast(srcIP, dstIP, row):
    ...
    elif ip1_1 > 223 or (ip1_4 == 192 and ip1_4 == 255):  # logic error ip1_4 == 192 and ip1_4==255 will never be true
        failed_multicast += 1
...
def string_totals(sample_name):
    counts = [['internal_ip', succeeded_one_ip_intern, failed_one_ip_intern],
              ['TCP_PORT', succeeded_tcp_80, failed_tcp_80],
              ['UDP_PORT', succeeded_udp_53, failed_udp_53],
              ['BYT_to_PKT', succeeded_byte_packet, failed_byte_packet],
              ['duration', succeeded_dur_one_packet, failed_dur_one_packet],
              ['netbios', succeeded_netbios, failed_netbios],
              ['dsn_21', succeeded_dsn21_check1, failed_dsn21_check1]
              ]
    output = ''

    for test_name, true_count, false_count in counts:
        tot_count = true_count + false_count

        # print(data_name, '='*10, file=f)
        if tot_count > 0:
            output += '%s, %s, %f, %f, %f\n' % (sample_name, test_name, true_count, tot_count, true_count / tot_count)
        else:
